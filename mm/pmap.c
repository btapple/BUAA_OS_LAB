...
{
    // the first address past the end of the uninitialized data segment, also known as BSS
    extern char end[];
    u_long      alloced_mem;
    ...
    if (clear)
    {
        // if here you find some problem, maybe you should refer to your linker script
...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
     ...
     * table. */
    // *pgdir_entryp contains a pointer to 4KB space
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    for (i = 0; i < VPN(size); i++)
    {
        ...
        // get the page table entry for va_temp
...
void mips_vm_init()
{
    ...
    /* Step 1: Allocate a page (4KB) for page directory(first level page table). */
...
void page_init(void)
{
    ...
    // the other: larger than used
    for (i = used; i < npage; i++)
    {
        pages[i].pp_ref = 0;
        // Insert before head is much quicker
...
int page_alloc(struct Page **pp)
{
     ...
     * Hint: use `bzero`. */
    bzero((void *)(page2kva(ppage_temp)), BY2PG); // clear the page content
    *pp = ppage_temp;
    return 0;
}
...
int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    pgtable = (void *)(KADDR(PTE_ADDR(*pgdir_entryp)));
    *ppte   = pgtable + PTX(va);
    return 0;
}
...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    // set pgtable_entry point to corresponding entry
    ...
    if (pgtable_entry != 0 && (*pgtable_entry & PTE_V) != 0)
    {
        ...
        {
            // if exists and matches, set valid and invalidate tlb
    ...
    /* Step 2: Update TLB. */
    /* as the page content has been modified, the TLB should be invalidated */
    tlb_invalidate(pgdir, va);
    ...
    *pgtable_entry = (page2pa(pp)) | PERM; // map
    pp->pp_ref++;
    return 0;
}
...
#ifdef BTAPPLE_MALLOC
// simplified malloc implementation
int __is_free(u_int va)
{
    if (page_lookup(curenv->env_pgdir, va, NULL) == 0)
        return 1;
    return 0;
}
...
void *__malloc(size_t n)
{
    ...
    u_int real_size          = ROUND(n + 8);
    u_int page_num           = VPN(real_size);
    u_int heap_position_temp = curenv->heap_position;

    if (curenv->heap_position + real_size >= USTACKTOP - BY2PG)
        return NULL;

    u_int va   = curenv->heap_position;
    int   flag = 0;

    while (va + real_size < USTACKTOP - BY2PG)
    {
        int cnt = 0;
        ...
        curenv->heap_position = va;
    }

    if (flag == 0)
    {
        curenv->heap_position = heap_position_temp;
        return NULL;
    }

    va = curenv->heap_position;
    for (i = 0; i < page_num; i++, va += BY2PG)
    {
        if ((r = page_alloc(&p)) < 0 ||
            (r = page_insert(curenv->env_pgdir, p, va, PTE_V | PTE_R)) < 0)
        {
            for (j = 1; j <= i; j++)
                page_remove(curenv->env_pgdir, va - j * BY2PG);
            return NULL;
        }
    }

    *((u_int *)curenv->heap_position)       = 114514;
    *((u_int *)(curenv->heap_position + 4)) = page_num;

    return curenv->heap_position + 8;
}
...
void __free(void *va)
{
    ...
    if (*((u_int *)curenv->heap_position) != 114514)
        return;

    page_num = *((u_int *)(curenv->heap_position + 4));
    for (i = 0; i < page_num; i++)
    {
