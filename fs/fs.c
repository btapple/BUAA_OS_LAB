...
u_int diskaddr(u_int blockno)
{
    if (super && blockno > super->s_nblocks)
        user_panic("diskaddr failed: blockno too large.\n");
    // if (!super)
    //     user_panic("diskaddr failed: super block not available.\n");
...
void free_block(u_int blockno)
{
    ...
    if (super == 0 || blockno <= 2 || blockno >= super->s_nblocks)
        return;
...
int walk_path(char *path, struct File **pdir, struct File **pfile, char *lastelem)
{
    ...
    // initialize pdir and pfile.
    // If failed, they should be null.
    if (pdir)
        *pdir = 0;
    ...
    *pfile = 0;
    ...
    // dir stores prev, file stores now.
    while (*path != '\0')
    {
        dir = file;
        p   = path;
        // [p, path] : file/directory name
        while (*path != '/' && *path != '\0')
            path++;
