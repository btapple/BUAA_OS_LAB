...
int map_segment(int child_envid, int fd, u_int va,
                u_int mem_size, u_int file_size, u_int offset, int perm)
{
    int   i, r;
    u_int off;
    void *blk;
    ...
    for (i = 0; i < mem_size; i += BY2PG)
    {
        if (i >= file_size)
        {
            if ((r = syscall_mem_alloc(child_envid, va + i, perm)) < 0)
                return;
        }
        else
        {
            if (perm & PTE_R)
            {
                if ((r = syscall_mem_alloc(0, USTACKTOP, PTE_V | PTE_R)) < 0)
                    return r;
                if ((r = seek(fd, offset + i)) < 0)
                    return r;
                if ((r = read(fd, USTACKTOP, MIN(BY2PG, file_size - i))) < 0)
                    return r;
                if ((r = syscall_mem_map(0, USTACKTOP, child_envid, va + i, perm)) < 0)
                    return r;
...
int usr_load_elf(int fd, Elf32_Phdr *ph, int child_envid)
{
    ...
    u_int vaddr       = ph->p_vaddr;
    u_int mem_size    = ph->p_memsz;
    u_int file_size   = ph->p_filesz;
    map_segment(child_envid, fd, vaddr, mem_size, file_size, file_offset, PTE_V | PTE_R);
...
int spawn(char *prog, char **argv)
{
    ...
    elf = (Elf32_Ehdr *)elf_buf;
    if (read(fd, elf_buf, sizeof(elf_buf)) != sizeof(elf_buf) ||
        !usr_is_elf_format(elf))
    {
        close(fd);
        writef("%s is not an executable file.", prog);
        return -E_NOT_EXEC;
    }

    if ((child_envid = syscall_env_alloc()) < 0)
    {
        writef("Unable to create a child process.\n");
        return child_envid;
    }

    init_stack(child_envid, argv, &esp);

    ph = (Elf32_Phdr *)(elf_buf + elf->e_phoff);
    for (i = 0; i < elf->e_phnum; i++, ph++)
    {
        if (ph->p_type != PT_LOAD)
            continue;
        if ((r = usr_load_elf(fd, ph, child_envid)) < 0)
            user_panic("user load elf file failed.\n");
    }
