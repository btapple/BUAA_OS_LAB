...
again:
    ...
    for (;;)
    {
        ...
        case '<':
            ...
            {
                writef("open %s for read failed: %e\n", t, fd);
                exit();
            }
            if (fd != 0)
            {
        ...
        case '>':
            ...
            {
                writef("open %s for write failed: %e\n", t, fd);
                exit();
            }
            if (fd != 1)
            {
        ...
        case '|':
            if ((r = pipe(p)) < 0)
            {
                writef("allocate pipe failed: %e\n", r);
                exit();
            }
            if ((r = fork()) < 0)
            {
                writef("fork failed: %e\n", r);
                exit();
            }
            if (!r)
            {
            ...
            break;
        default:
            user_panic("unknown value returned from gettoken: %d", c);
            break;
        }
    }
