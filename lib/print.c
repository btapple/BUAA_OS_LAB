...
// if the length is too short or too long, fatal error message should be printed.
// the length is 1 bigger than string-length, due to the '\0' mark
#define OUTPUT(arg, s, l)                                                 \
    ...
    for (;;)
    {
        ...
        /* we found a '%', and consume it */
        fmt++;

        /* according to printf standard, check for lad first */
        if (*fmt == '-')
        {
            ladjust = 1;
            fmt++;
