...
int env_alloc(struct Env **new, u_int parent_id)
{
    ...
    // if it is empty, return error code
    ...
    // e->env_tf.regs[29] = USTACKTOP - 2 * sizeof(size_t);
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    // alloc the unaligned part
    i = 0;
    if (offset)
    {
        // duplicate code, but I am lazy.
        if ((r = page_alloc(&p)) < 0)
            return r;
    ...
    // alloc the aligned part
    for (; i + BY2PG < bin_size; i += BY2PG, va += BY2PG, bin += BY2PG)
    {
        ...
        bcopy(bin, (void *)(page2kva(p)), BY2PG);
    }
    ...
    if (bin_size > i)
    {
        ...
        bzero((void *)(page2kva(p)), BY2PG);
        bcopy(bin, (void *)(page2kva(p)), bin_size - i);
        va += BY2PG;
        i += BY2PG;
    }
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
    ...
    // if we walk the pgdir, we will get p
